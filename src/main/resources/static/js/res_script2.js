var resScript = (function(){
    ...
    var $clickSeat; //선택한 좌석
    var timeArr=[]; //좌석에서 선택한 시간 
    var locationTxt, storeTxt, seatTxt, lockerTxt, dayTxt; // 디테일에 보여질 텍스트값 (지역, 지점, 좌석, 사물함, 날짜)
    var resDetailArr=[];
    var resListNum = 1;
    var addSeat=0, addRoom=0, addLock=0; //resAddClick()
    ...
    return{
        ...
        seatClick : function($clickSeat){//좌석선택시 이벤트
            if($(".reserveCont select.location").val() == "none" || $(".reserveCont select.store").val() == "none"){
                ...
                if(!$clickSeat.hasClass("on")) {
                    // 리셋 
                    timeArr=[];
                    $(".resTime .scrollD a").removeClass("on"); // 시간 선택 reset 
                    $(".slcDeTail .total .pay").html("0"); //총 금액 0
                    $(".slcDeTail p").not(".location").not(".store").not(".date").find(".data").empty(); //디테일에서 지역, 지점 data 빼고 삭제 
                    $(".seatDiv .seat").removeClass("on");
        ...
        resAddClick : function(){ // 예약 추가 버튼 클릭 시 
            if(timeArr.length != 0 || dayTxt != null){
                ...
                for(var r=0; r<timeArr.length; r++){
                    resDetailArr.push({
                        loc:$(".slcDeTail .location .data").text(),
                        store:$(".slcDeTail .store .data").text(),
                        seat:$(".slcDeTail .seat .data").text(),
                        date:$(".slcDeTail .date .data").text(),
        ...
        arrList : function(){
            ...
            for(var l=0; l<resDetailArr.length; l++){
                ...
                $html+="</div><a href='lavascript:' class='del'>삭제</a></div>";
        ...
        resState : function(store_no,res_date,rcnt,dcnt,bcnt){// 예약 된 좌석,룸 정보 로드 후에 좌석 선택 
            $.ajax({
				type: "POST",
				url: "../reserveState",
                data:{
        ...
        resChange : function(){// 지점 선택, 달력 날짜 선택 
            ...
            $(document).on("click", ".calTable table td a", function(){ //달력 날짜 선택시 
                if($(".reserveCont select.store").val() !== "none"){
                    if(!$(this).parents("td").hasClass("before") && !$(this).parents("td").hasClass("empty")){
                        ...
                        store_no = $(".reserveCont select.store option:selected").text();
                        res_date = selY+"-"+selM+"-"+selD;
                        
                        for(var i=0; i<resStoreArr.length; i++){
                            if(res_date == resStoreArr[i].res_date){
                                resScript.resState(store_no,res_date,resStoreArr[i].rcnt,resStoreArr[i].dcnt,resStoreArr[i].bcnt);
                            }
                        }
                    }
